...
public class CropProductionService {
    ...
    public byte[] createExcelSummaryReport(LocalDate from, LocalDate to) {
        ...
        Map<String, List<ProducerData>> resultMap = new HashMap<>();
        for (Entry<Long, OperationalReport> entry : recipientIdToLastReportMap.entrySet()) {
            ...
            String distName = dictionaryCodeById(codesMap, DISTRICT, recipient.getDistrictId());
            resultMap.computeIfAbsent(distName, k -> new ArrayList<>()).add(producerData);
        }

        List<DistrictData> result = resultMap.entrySet().stream()
            .map(entry -> new DistrictData(entry.getKey(), entry.getValue()))
            .toList();
