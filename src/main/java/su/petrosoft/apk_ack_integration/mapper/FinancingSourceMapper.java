...
public class FinancingSourceMapper {
    ...
    public FinancingSource toEntity(InstanceDto dto, Map<Dictionary, Map<String, Long>> codesMap) {
        ...
        return new CreateInstanceRequestDto(
            InstanceDto.builder()
                ...
                .attributes(List.of(
                    ...
                    new LinkedAttribute(KVSR_ATTR,
                        dictionaryIdByCode(codesMap, KVSR, fs.getKvsr())),
                    new LinkedAttribute(KFSR_ATTR,
                        dictionaryIdByCode(codesMap, KFSR, fs.getKfsr())),
                    new LinkedAttribute(KCSR_ATTR,
                        dictionaryIdByCode(codesMap, KCSR, fs.getKcsr())),
                    new LinkedAttribute(KVR_ATTR, dictionaryIdByCode(codesMap, KVR, fs.getKvr())),
                    new LinkedAttribute(KOSGU_ATTR,
                        dictionaryIdByCode(codesMap, KOSGU, fs.getKosgu())),
                    new LinkedAttribute(DOPEK_ATTR,
                        dictionaryIdByCode(codesMap, DOPEK, fs.getDopEk())),
                    new LinkedAttribute(DOPFK_ATTR,
                        dictionaryIdByCode(codesMap, DOPFK, fs.getDopFk())),
                    new LinkedAttribute(DOPKR_ATTR,
                        dictionaryIdByCode(codesMap, DOPKR, fs.getDopKr())),
                    new LinkedAttribute(PURPOSE_ATTR,
                        dictionaryIdByCode(codesMap, PURPOSE, fs.getPurpose())),
                    new LinkedAttribute(OWNERSHIP_FORM_ATTR,
                        dictionaryIdByCode(codesMap, OWNERSHIP_FORM, fs.getOwnershipForm())),
